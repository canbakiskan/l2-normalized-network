...
class ResidualUnit_l2_normalized(nn.Module):

    def __init__(self, in_channel, out_channel, stride=1, activate_before_residual=False):
        super(ResidualUnit_l2_normalized, self).__init__()

        self.bn1 = nn.BatchNorm2d(in_channel)
        self.conv1 = Conv2d_l2_normalized(in_channel, out_channel, kernel_size=3,
                                          stride=stride, padding=1, bias=False)
        torch.nn.init.xavier_normal_(self.conv1.weight)
        self.bn2 = nn.BatchNorm2d(out_channel)
        self.conv2 = Conv2d_l2_normalized(out_channel, out_channel, kernel_size=3,
                                          stride=1, padding=1, bias=False)
        torch.nn.init.xavier_normal_(self.conv2.weight)
        self.activate_before_residual = activate_before_residual
        self.in_channel = in_channel
...
class ResNet_l2_normalized(nn.Module):

    def __init__(self, num_outputs=10):
        super(ResNet_l2_normalized, self).__init__()
        ...
        self.block1 = self.build_block(
            ResidualUnit_l2_normalized, 5, filters[0], filters[1], strides[0], True)
        self.block2 = self.build_block(
            ResidualUnit_l2_normalized, 5, filters[1], filters[2], strides[1], False)
        self.block3 = self.build_block(
            ResidualUnit_l2_normalized, 5, filters[2], filters[3], strides[2], False)
...
class ResNetWide_l2_normalized(nn.Module):

    def __init__(self, num_outputs=10):
        super(ResNetWide_l2_normalized, self).__init__()
